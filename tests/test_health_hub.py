...
class SchemaTests(unittest.TestCase):
    def test_valid_schema_and_nested_system(self):
        ...
        self.assertEqual(
            systems[0].children[0].checks[0].params["warning_after"], timedelta(minutes=5)
        )
    ...
    def test_rejects_unknown_check_type_including_template(self):
        with self.assertRaisesRegex(ConfigValidationError, "unsupported type"):
            validate_config(
                base_config([{"id": "bad", "type": "template", "entity_id": "sensor.example"}])
            )
    ...
    def test_rejects_duplicate_ids_and_invalid_ranges(self):
        with self.assertRaises(ConfigValidationError):
            validate_config(
                base_config(
                    [
                        {"id": "same", "type": "availability", "entity_id": "sensor.one"},
                        {"id": "same", "type": "availability", "entity_id": "sensor.two"},
                    ]
                )
            )
        with self.assertRaises(ConfigValidationError):
            ...
            validate_config(
                {
                    "systems": {
                        "one": {
                            "name": "One",
                            "checks": [
                                {"id": "a", "type": "availability", "entity_id": "sensor.a"}
                            ],
                        },
                        "two": {
                            "name": "Two",
                            "children": {
                                "one": {
                                    "name": "Duplicate",
                                    "checks": [
                                        {"id": "b", "type": "availability", "entity_id": "sensor.b"}
                                    ],
                                }
                            },
                        },
                    ...
                    [
                        {
                            ...
                            "entity_id": "sensor.a",
                            "max_age_warning": "5m",
                            "active_when": {"entity_id": "binary_sensor.mode", "state": []},
                        }
                    ]
                )
            )
...
class CheckTests(unittest.TestCase):
    ...
    def test_freshness_uses_last_updated_not_last_changed(self):
        ...
        self.assertEqual(
            check.evaluate(
                state, {"sensor.a": state}, CheckRuntimeState(), T0 + timedelta(minutes=4)
            ).status,
            STATUS_OK,
        )
        self.assertEqual(
            check.evaluate(
                state, {"sensor.a": state}, CheckRuntimeState(), T0 + timedelta(minutes=5)
            ).status,
            STATUS_WARNING,
        )
        self.assertEqual(
            check.evaluate(
                state, {"sensor.a": state}, CheckRuntimeState(), T0 + timedelta(minutes=10)
            ).status,
            STATUS_CRITICAL,
        )
    ...
    def test_stale_value_window_delta_and_active_when(self):
        ...
        self.assertEqual(
            check.evaluate(state, {"sensor.lux": state}, runtime, T0).status, STATUS_UNKNOWN
        )
        self.assertEqual(
            check.evaluate(state, {"sensor.lux": state}, runtime, T0 + timedelta(hours=1)).status,
            STATUS_WARNING,
        )
        ...
        result = check.evaluate(
            state, {"sensor.lux": state}, runtime, T0 + timedelta(hours=2, minutes=1)
        )
        self.assertEqual(result.status, STATUS_OK)
        ...
        result = check.evaluate(
            state, {"sensor.lux": state}, runtime, T0 + timedelta(hours=3, minutes=1)
        )
        ...
        result = build_check(definition).evaluate(
            state,
            {"sensor.lux": state, "binary_sensor.day": snapshot("binary_sensor.day", "off")},
            CheckRuntimeState(),
            T0,
        )
    ...
    def test_battery_requires_percent_and_thresholds(self):
        ...
        check = build_check(self.definition(raw))
        self.assertEqual(
            check.evaluate(
                snapshot("sensor.battery", "18", unit="%"), {}, CheckRuntimeState(), T0
            ).status,
            STATUS_WARNING,
        )
        self.assertEqual(
            check.evaluate(
                snapshot("sensor.battery", "10", unit="%"), {}, CheckRuntimeState(), T0
            ).status,
            STATUS_CRITICAL,
        )
        self.assertEqual(
            check.evaluate(
                snapshot("sensor.battery", "18", unit="V"), {}, CheckRuntimeState(), T0
            ).status,
            STATUS_UNKNOWN,
        )

    def test_numeric_range_and_expected_binary_states(self):
        numeric = self.definition(
            {
                "id": "temp",
                "type": "numeric_range",
        ...
        self.assertEqual(
            check.evaluate(snapshot("sensor.temp", "20"), {}, CheckRuntimeState(), T0).status,
            STATUS_OK,
        )
        self.assertEqual(
            check.evaluate(snapshot("sensor.temp", "26"), {}, CheckRuntimeState(), T0).status,
            STATUS_WARNING,
        )
        self.assertEqual(
            check.evaluate(snapshot("sensor.temp", "31"), {}, CheckRuntimeState(), T0).status,
            STATUS_CRITICAL,
        )
        ...
        self.assertTrue(result.defer)
        result = build_check(expected).evaluate(
            snapshot("switch.safe", "on"), {}, runtime, T0 + timedelta(minutes=5)
        )
        self.assertEqual(result.status, STATUS_WARNING)
...
class ManagerTests(unittest.IsolatedAsyncioTestCase):
    async def manager(self, config, backend=None):
        ...
        manager = HealthManager(
            ...
            HealthHubStore(backend or MemoryBackend()),
    ...
    async def seed(self, manager, values):
        await manager.async_initialize(
            [snapshot(entity, value) for entity, value in values.items()]
        )
    ...
    async def test_hierarchy_required_optional_and_worst_status(self):
        ...
        await manager.async_handle_entity_update(
            snapshot("sensor.optional", "unavailable"), entity_id="sensor.optional"
        )
        self.assertEqual(manager.system_runtime["optional"].status, STATUS_OK)
    ...
    async def test_events_flapping_recovery_and_no_duplicate_updates(self):
        ...
        await manager.async_handle_entity_update(
            snapshot("sensor.a", "11", at=T0 + timedelta(minutes=1)), entity_id="sensor.a"
        )
        self.assertFalse(any(event[0] == EVENT_ISSUE_OPENED for event in events))
        manager.test_clock["now"] = T0 + timedelta(minutes=2)
        await manager.async_handle_entity_update(
            snapshot("sensor.a", "1", at=T0 + timedelta(minutes=2)), entity_id="sensor.a"
        )
        self.assertFalse(any(event[0] == EVENT_ISSUE_OPENED for event in events))
        manager.test_clock["now"] = T0 + timedelta(minutes=3)
        await manager.async_handle_entity_update(
            snapshot("sensor.a", "11", at=T0 + timedelta(minutes=3)), entity_id="sensor.a"
        )
        ...
        await manager.async_handle_entity_update(
            snapshot("sensor.a", "1", at=T0 + timedelta(minutes=9)), entity_id="sensor.a"
        )
        ...
        self.assertGreaterEqual(
            len([event for event in events if event[0] == EVENT_STATUS_CHANGED]), 2
        )

    async def test_store_restore_preserves_open_issue_and_observations(self):
        backend = MemoryBackend()
        config = base_config(
            [{"id": "range", "type": "numeric_range", "entity_id": "sensor.a", "warning_above": 10}]
        )
    ...
    async def test_invalid_store_recovers_safely(self):
        ...
        manager, _ = await self.manager(
            base_config([{"id": "a", "type": "availability", "entity_id": "sensor.a"}]), backend
        )
    ...
    async def test_deadlines_and_500_check_dependency_index(self):
                ...
                "checks": [
                    {
                        "id": "available",
                        "type": "availability",
                        "entity_id": f"sensor.node_{index}",
                        "warning_after": "5m",
                    }
                ],
            }
        ...
        await manager.async_initialize(
            [snapshot(f"sensor.node_{index}", "1") for index in range(500)]
        )
    ...
    async def test_diagnostics_is_compact_and_has_no_snapshot_attributes(self):
        manager, _ = await self.manager(
            base_config([{"id": "a", "type": "availability", "entity_id": "sensor.a"}])
        )
        await self.seed(manager, {"sensor.a": "1"})
