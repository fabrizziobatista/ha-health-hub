...
class ExpectedStateCheck(BaseCheck):
    ...
    def evaluate(self, snapshot, snapshots, runtime, now: datetime) -> CheckResult:
        ...
        if is_invalid(snapshot):
            ...
            return self.unknown(
                "Expected-state entity is unavailable", value=getattr(snapshot, "state", None)
            )
        ...
        if critical_after is not None and elapsed >= critical_after:
            return CheckResult(
                "critical", f"Unexpected state {snapshot.state!r}", value=snapshot.state
            )
        if elapsed >= warning_after:
            return CheckResult(
                "warning",
                f"Unexpected state {snapshot.state!r}",
                value=snapshot.state,
                next_deadline=(runtime.bad_since + critical_after) if critical_after else None,
            )
        return CheckResult(
            "ok",
            "Unexpected state is within grace period",
            value=snapshot.state,
            defer=True,
            next_deadline=runtime.bad_since + warning_after,
        )
