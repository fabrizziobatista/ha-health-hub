...
class FreshnessCheck(BaseCheck):
    ...
    def evaluate(self, snapshot, snapshots, runtime, now: datetime) -> CheckResult:
        ...
        if is_invalid(snapshot) or snapshot.last_updated is None:
            return self.unknown("No valid last_updated timestamp", value=getattr(snapshot, "state", None))
        ...
        if warning is not None and age >= warning:
            ...
            return CheckResult("warning", f"No update for {age}", value=snapshot.state, next_deadline=deadline)
        deadlines = [snapshot.last_updated + value for value in (warning, critical) if value is not None]
        return CheckResult("ok", "Entity is fresh", value=snapshot.state, next_deadline=min(deadlines) if deadlines else None)
