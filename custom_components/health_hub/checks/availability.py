...
class AvailabilityCheck(BaseCheck):
    ...
    def evaluate(self, snapshot, snapshots, runtime, now: datetime) -> CheckResult:
        ...
        if critical_after is not None and elapsed >= critical_after:
            return CheckResult(
                "critical",
                "Entity unavailable beyond critical grace",
        ...
        if elapsed >= warning_after:
            return CheckResult(
                ...
                "Entity unavailable",
                value=getattr(snapshot, "state", None),
                next_deadline=(runtime.invalid_since + critical_after) if critical_after else None,
            )
        return CheckResult(
            "ok",
            "Entity unavailability is within grace period",
            value=getattr(snapshot, "state", None),
            defer=True,
            next_deadline=runtime.invalid_since + warning_after,
        )
