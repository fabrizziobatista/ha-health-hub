...
from .const import (
    ...
    CHECK_FRESHNESS, CHECK_NUMERIC_RANGE, CHECK_STALE_VALUE, CHECK_TYPES,
    CONF_ACTIVE_WHEN, CONF_CHECKS, CONF_CHILDREN, CONF_ENTITY_ID, CONF_ICON,
    CONF_ID, CONF_MINIMUM_STATE_DURATION, CONF_NAME, CONF_OPTIONAL, CONF_SYSTEMS,
    CONF_TYPE,
)
...
def _active_when(value: object) -> tuple[str, tuple[str, ...]] | None:
    ...
    states = (raw_state,) if isinstance(raw_state, str) else tuple(raw_state) if isinstance(raw_state, list) else ()
...
def _validate_common(raw: Mapping[str, Any], system_id: str, seen: set[str]):
    ...
    minimum = parse_duration(raw.get(CONF_MINIMUM_STATE_DURATION, "0s"), f"check {check_id}.minimum_state_duration")
    return check_id, check_type, entity_id, optional, minimum, _active_when(raw.get(CONF_ACTIVE_WHEN))
...
def _validate_check(raw: object, system_id: str, seen: set[str]) -> CheckDefinition:
    ...
    check_id, check_type, entity_id, optional, minimum, active_when = _validate_common(raw, system_id, seen)
    common = {CONF_ID, CONF_TYPE, CONF_ENTITY_ID, CONF_OPTIONAL, CONF_MINIMUM_STATE_DURATION, CONF_ACTIVE_WHEN}
    ...
    if check_type in (CHECK_AVAILABILITY, CHECK_EXPECTED_STATE, CHECK_BINARY_STATE):
        ...
        if critical is not None and critical < warning:
            raise ConfigValidationError(f"check {check_id}.critical_after must not be before warning_after")
        ...
        if check_type in (CHECK_EXPECTED_STATE, CHECK_BINARY_STATE):
            ...
            if check_type == CHECK_BINARY_STATE and expected not in ("on", "off"):
                raise ConfigValidationError(f"binary_state check {check_id}.expected must be on or off")
            if check_type == CHECK_EXPECTED_STATE:
                ...
                if not values or any(not isinstance(item, str) or not item for item in values):
                    raise ConfigValidationError(f"expected_state check {check_id}.expected must be a string or non-empty list")
        ...
        if set(params).difference({"max_age_warning", "max_age_critical"}):
            raise ConfigValidationError(f"check {check_id} has unsupported parameters")
        warning, critical = _optional_duration(params, "max_age_warning"), _optional_duration(params, "max_age_critical")
        if warning is None and critical is None:
            raise ConfigValidationError(f"freshness check {check_id} needs max_age_warning and/or max_age_critical")
        if warning is not None and critical is not None and critical < warning:
            raise ConfigValidationError(f"check {check_id}.max_age_critical must not be before max_age_warning")
        ...
        if window is None or window.total_seconds() <= 0:
            raise ConfigValidationError(f"stale_value check {check_id}.observation_window must be positive")
        if isinstance(changes, bool) or not isinstance(changes, int) or changes < 1:
            raise ConfigValidationError(f"stale_value check {check_id}.minimum_changes must be an integer >= 1")
        delta = _number(params.get("minimum_delta"), f"stale_value check {check_id}.minimum_delta")
        if delta < 0:
            raise ConfigValidationError(f"stale_value check {check_id}.minimum_delta cannot be negative")
        ...
        if not 0 <= critical <= warning <= 100:
            raise ConfigValidationError(f"battery check {check_id} needs 0 <= critical_below <= warning_below <= 100")
        ...
        if set(params).difference(allowed) or not params:
            raise ConfigValidationError(f"numeric_range check {check_id} needs one or more numeric limits")
        for key in tuple(params):
            params[key] = _number(params[key], f"numeric_range check {check_id}.{key}")
        if "warning_below" in params and "critical_below" in params and params["critical_below"] > params["warning_below"]:
            raise ConfigValidationError(f"numeric_range check {check_id}.critical_below must be <= warning_below")
        if "warning_above" in params and "critical_above" in params and params["critical_above"] < params["warning_above"]:
            raise ConfigValidationError(f"numeric_range check {check_id}.critical_above must be >= warning_above")
...
def _validate_systems(raw: object, seen: set[str]) -> tuple[SystemDefinition, ...]:
    ...
    for system_id, value in raw.items():
        ...
        if stable_id in seen:
            raise ConfigValidationError(f"duplicate system id {stable_id}; ids must be globally unique")
        seen.add(stable_id)
        if not isinstance(value, Mapping) or set(value).difference({CONF_NAME, CONF_ICON, CONF_CHECKS, CONF_CHILDREN}):
